...
use iris_ast::{Ast, Spanned, item::ItemKind};
use iris_diagnostic::{Diagnostic, codes::UNEXPECTED_TOKEN};
use iris_span::{Span, source_file::SourceFileId};
use iris_syntax::{Token, TokenKind};
pub use parse::*;

use crate::{diagnostic::map_parse_error, error::ParseError, stream::TokenStream};
...
impl<'a> Parser<'a> {
    ...
    pub(crate) fn stream_mut(&mut self) -> &mut TokenStream {
        &mut self.stream
    }
    ...
    pub(crate) fn expect_bump_with_flag(
        &mut self,
        kind: TokenKind,
        emmit_error: bool,
    ) -> Option<&Token> {
    ...
    pub(crate) fn expect_bump(&mut self, kind: TokenKind) -> Option<&Token> {
        self.expect_bump_with_flag(kind, true)
    }

    pub(crate) fn try_expect(&mut self, kinds: &[TokenKind]) -> bool {
    ...
    pub(crate) fn report_error(&mut self, error: ParseError, span: Span) {

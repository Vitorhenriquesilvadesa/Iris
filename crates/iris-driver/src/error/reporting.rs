...
use iris_diagnostic::{Diagnostic, DiagnosticLabel, DiagnosticSeverity};
use iris_span::source_file::SourceFile;
use iris_span::source_map::SourceMap;
...
impl<'a> DiagnosticRenderer<'a> {
    ...
    pub fn emit(&self, diag: &Diagnostic) {
        ...
        if let (Some(file_id), Some(span)) = (diag.file, diag.primary_span)
            && let Some(file) = self.map.load_by_id(file_id)
        {

use common::{
    ast::{
        ...
        item::{
            AstTypeBase, AstTypeInfo, AstTypeModifier, FunctionDef, GenericParam, ImplDef,
            ItemKind, MetaArgument, MetaDataUsage, TypeDef, TypeName,
        },
        statement::LetStmt,
    },
    span::Span,
...
impl<'a> Parser<'a> {
    ...
    fn parse_function(&mut self) -> ParseResult<Item> {
        ...
        let params = self.parse_separated_elements(TokenKind::Comma, |p| p.parse_param())?;
    ...
    fn parse_param(&mut self) -> ParseResult<Spanned<Param>> {
        ...
        let kind = self.parse_type_info()?;
        let kind_span = kind.span;

        Some(Spanned::new(
            Param { name, kind },
            name_span.merge(&kind_span),
        ))
    }
